  ...
  return (
    <div className="flex flex-col gap-6 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
       ...
       <div className="flex flex-col gap-1">
       <label className="text-xs text-gray-500 uppercase tracking-wide">Imagen</label>
       <input
            type="file"
            accept="image/*"
            onChange={async (e) => {
      ...
      try {
        ...
        setForm(prev => ({ ...prev, image: url }))
          ...
          className="text-sm text-gray-400 file:mr-3 file:px-3 file:py-1.5 file:bg-[#6A1E55] file:text-white file:border-0 file:text-xs file:cursor-pointer hover:file:bg-[#A64D79] transition"
          />
          {uploading && <p className="text-xs text-gray-500">Subiendo imagen...</p>}
          {form.image && (
          <img src={form.image} alt="preview" className="w-20 h-20 object-cover mt-1" />
          )}
     </div>
